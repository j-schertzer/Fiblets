...
bool rejectFiber(const in vec3 pt, const uint fid)
{
...
void main(void)
{
	...
	if (!rejectFiber(decompressedPoints[0], fiberID) || nbPtPack == 0 || !critRender) 
		return;

	mat4 Mi = retreiveFirstTransform(decompressedPoints);
	gl_Position = projCamMultMV * Mi[3];
